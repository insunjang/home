...
class ChannelIntro extends PureComponent {
    static propTypes = {
        ...
        diplayLimitedDateMsg:PropTypes.bool.isRequired,
        ...
        return (
            <Text style={style.message}>
                {intl.formatMessage({
                    ...
                    defaultMessage: '모바일에서는 3일이전의 메세지를 볼수 없습니다.',
                })}
            </Text>
        );
    };
    ...
    render() {
        const {currentChannel, isLoadingPosts, theme, diplayLimitedDateMsg} = this.props;
        const style = getStyleSheet(theme);
        const channelType = currentChannel.type;

        if (isLoadingPosts && !diplayLimitedDateMsg) {
            return (
        ...
        if(diplayLimitedDateMsg){
          return (
              <View style={style.container}>
                  <View style={style.contentContainer}>
                      {this.buildEndofListContent()}
                  </View>
              </View>
          );
        }
