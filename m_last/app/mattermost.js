...
export default class Mattermost {
    constructor() {
      ...
      console.log('!!!!!userId : ' + this.userId + '   !!!!!epId : ' + this.epId);
      console.log('!!!!!baseUrl : ' + this.baseUrl + '   !!!!!Url : ' + this.ssoUrl);
    ...
    getInfoFromBAS = async (isready) => {
      try {
        const {u, e, base, sso} = await mattermostManaged.getInfoFromBAS();
        this.userId  = u;
        this.epId  = e;
        this.baseUrl = base;
        this.ssoUrl = sso;
        console.log('***userId : ' + u + '   ***epId : ' + e);
        console.log('@@@baseUrl : ' + base + '   @@@Url : ' + sso);
    ...
    getUserInfo = async (isready) => {
      try {
          const {u, e} = await mattermostManaged.getUserInfo();
          console.log('***userId : ' + u + '   ***epId : ' + e);
          this.userId  = u;
          this.epId  = e;
        } catch (error) {
      ...
      getUrl = async (isready) => {
        try {
          const {base, sso} = await mattermostManaged.getUrl();
          console.log('@@@baseUrl : ' + base + '   @@@Url : ' + sso);
          this.baseUrl = base;
          this.ssoUrl = sso;
        } catch (error) {
        ...
        if (state.views.root.hydrationComplete) {
            ...
            if (Platform.OS === 'android') {
                ...
                Promise.resolve(Navigation.isAppLaunched()).then((appLaunched) => {
                    if (appLaunched /*&& this.userId && this.epId && this.baseUrl && this.ssoUrl*/) {
                ...
                //if(this.userId && this.epId && this.baseUrl && this.ssoUrl){
                  this.launchApp();
              //}
            }
        }
