...
class SSO extends PureComponent {
    ...
    constructor(props) {
        ...
        case ViewTypes.SAML:
            ...
            //this.loginUrl = `https://www.samsungsmartoffice.net/kms/jsp/saml/mattermost/mobile.jsp?redirect_to=/`;
            break;
        }
    }
    ...
    onLoadEnd = (event) => {
        ...
        //if (url.includes(this.completedUrl)) {
                CookieManager.get(this.props.baseUrl).then((res) => {
                ...
                if (token) {
                    ...
                    setStoreFromLocalData({url: this.props.baseUrl, token}).
    ...
    render() {
        const {theme, userId, epId} = this.props;
        const {error, renderWebView, onMessage, jsCode, scalePagesToFit/*, userId, epId*/} = this.state;
        const style = getStyleSheet(theme);
        let content;
        if (!renderWebView || !userId ||  !epId)  {
